...
const field: React.CSSProperties = {
  ...
  fontSize: '13px', fontWeight: 600, color: '#44403c',
};
const input: React.CSSProperties = {
  borderRadius: '10px', border: '1px solid #d6d3d1',
  background: 'white', padding: '9px 14px',
  fontSize: '13px', color: '#1c1917', outline: 'none', width: '100%',
};
    ...
    <div style={{ padding: '24px 32px', maxWidth: '700px' }}>
      ...
      <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #e7e5e4', padding: '24px', boxShadow: '0 1px 4px rgba(28,25,23,0.05)' }}>
        <form action={formAction} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
          ...
          <div style={{ borderTop: '1px solid #f0eeec', paddingTop: '14px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <p style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.2em', color: '#a8a29e', margin: 0 }}>Social &amp; Links</p>
          ...
          <div style={{ borderTop: '1px solid #f0eeec', paddingTop: '14px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <p style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.2em', color: '#a8a29e', margin: 0 }}>Education</p>
              <button type="button" onClick={addEdu} style={{ fontSize: '12px', color: '#b87a38', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>+ Add Education</button>
            </div>
            {edu.map((item, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1.2fr 1.2fr 0.6fr 40px', gap: '10px', alignItems: 'flex-end', background: '#fafafa', padding: '10px', borderRadius: '8px' }}>
                ...
                <button type="button" onClick={() => removeEdu(i)} style={{ padding: '8px', color: '#b91c1c', background: 'none', border: 'none', cursor: 'pointer' }}>✕</button>
              </div>
            ))}
          </div>

          <div style={{ borderTop: '1px solid #f0eeec', paddingTop: '14px' }}>
            <p style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.2em', color: '#a8a29e', margin: '0 0 10px' }}>Professional Summary</p>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', paddingTop: '4px' }}>
            <button style={{ padding: '10px 24px', borderRadius: '999px', background: '#1c1917', color: 'white', fontSize: '13px', fontWeight: 600, border: 'none', cursor: isPending ? 'not-allowed' : 'pointer', opacity: isPending ? 0.6 : 1 }} disabled={isPending}>
              {isPending ? 'Saving…' : 'Save profile'}
            </button>
            {state.status === 'success' && <span style={{ fontSize: '13px', color: '#15803d', fontWeight: 600 }}>✓ Saved</span>}
            {state.status === 'error' && <span style={{ fontSize: '13px', color: '#b91c1c' }}>{state.message ?? 'Save failed'}</span>}
          </div>
        </form>
      </div>
    </div>
